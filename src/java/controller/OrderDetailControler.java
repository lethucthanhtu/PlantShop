...
public class OrderDetailControler extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            Map<Integer, Integer> orderCart = OrderDAO.getOrderDetailByOrderID(id);
            List<Plant> plants = new ArrayList<>();

            for (int i : orderCart.keySet()) {
                Plant plant = PlantDAO.getDetail(i);
                plants.add(plant);
            }

            request.setAttribute(Attribute.PLANTS, plants);
            request.setAttribute(Attribute.ORDER, orderCart);

            request.getRequestDispatcher(Page.ORDER_DETAIL).forward(request, response);
